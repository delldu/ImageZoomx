...
if __name__ == "__main__":
    ...
    def export_onnx():
        ...
        with torch.no_grad():
            torch_output = model(dummy_encoder_input, dummy_output_size)
        ...
        torch.onnx.export(
            model,
            (dummy_encoder_input, dummy_output_size),
            onnx_file_name,
            input_names=input_names,
            output_names=output_names,
            verbose=True,
    ...
    def verify_onnx():
        ...
        onnxruntime_inputs = {
            onnxruntime_engine.get_inputs()[0].name: to_numpy(dummy_encoder_input),
            onnxruntime_engine.get_inputs()[1].name: to_numpy(dummy_output_size),
        }
        onnxruntime_outputs = onnxruntime_engine.run(None, onnxruntime_inputs)
    ...
    if args.verify:
        ...
        print(
            "model does not been tested here for onnx not support grid_sampler so far !"
        )
